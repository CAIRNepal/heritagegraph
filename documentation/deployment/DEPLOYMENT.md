...
- [Dokploy compose and CI redeploy hook](#dokploy-compose-and-ci-redeploy-hook)
...
**[Dokploy](DOKPLOY.md)** (`docker-compose-dokploy.yml`) and
**[Coolify](deploy_on_coolify.md)** — both terminate TLS and route to
the containers; follow them for secrets, domains, and migrations.
...
Active compose services: `postgres`, `traefik`, `backend`, `frontend`, `landing`, `redis`, `oxigraph`. OCR worker is suspended.
