...
from heritage_graph.apps.heritage_data.models import Submission, CulturalHeritage  
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--user-id', type=int, required=True, help='ID of contributor user')
    ...
    def handle(self, *args, **options):
        ...
        if not csv_files:
            self.stdout.write(
                self.style.WARNING('No CSV files found in directory')
            )
            return
    ...
    def create_submission(self, resource_id, rows, contributor):
        ...
        monument_name = next(
            (row['Value'].strip() for row in rows if row['Label'].strip() == 'Monument name'), 
            None
        )
        ...
        for row in rows:
            ...
            if field_name:
                ...
                if field_name in ['description', 'Description_in_Nepali', 'Short_description']:
                    if value:
                        submission_data[field_name] = value
                # Handle object-specific fields
                elif label in ['Name', 'Object ID number', 'Object type', 'Object material', 
                               'Object location', 'Date (BCE/CE)', 'Commentary']:
        ...
        try:
            ...
            filtered_data = {k: v for k, v in submission_data.items() if k in valid_fields or k == 'contribution_data'}
            ...
            if created:
                self.stdout.write(self.style.SUCCESS(f'Created submission {resource_id}'))
            else:
                self.stdout.write(self.style.SUCCESS(f'Updated submission {resource_id}'))

        except Exception as e:
            self.stdout.write(self.style.ERROR(f'Error processing {resource_id}: {str(e)}'))
